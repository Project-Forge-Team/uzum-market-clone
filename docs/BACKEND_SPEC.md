...
Документ для backend-разработчика. Фронтенд (Next.js 16, App Router) **готов и менять его нельзя** —
вся логика в UI уже написана, остаётся заменить источник данных: сейчас это `route handlers` +
JSON-файл `.data/db.json` в песочнице фронтенда. Нужно поднять настоящий бэкенд, который отдаёт
**тот же контракт**, и фронт переключится на него (переменная окружения + 1 файл на сервере).
...
* Сид: **10 категорий, 10 магазинов, 42 товара, 84 отзыва, 1 демонстрационный заказ** — готовый
  датасет лежит во фронтенде в `lib/server/catalog.json`, можно грузить напрямую. Товары ссылаются
  на картинки `/products/gen/*.svg` — их тоже надо отдать (либо скопировать в `MEDIA_ROOT`, либо
  позволить фронту дотягиваться до `public/`).
...
## 10. Что меняем во фронтенде после твоей готовности (это моя часть, тебе на заметку)
...
6. **JWT vs cookie-session**: фронт заточен под куки + CSRF. Если будешь JWT, придётся менять
   `lib/api.ts` и `lib/server/auth.ts` на фронте — скажи заранее.
